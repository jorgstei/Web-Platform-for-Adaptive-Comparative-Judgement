<script>
    ...
    */
    ...
    console.log("loginPage changePassword: ", changePassword);
    ...
    const login = () => {
        ...
        if (formIsValid) {
            userService
                ...
                .catch((err) => {
                    swal("An error occured while trying to log in", err, "error");
                });
        } else {
    ...
    function validateEmail(v) {
        return email_regex.test(email) || "Please make sure this is a valid email address.";
    }
    ...
    function checkChangePasswordFields() {
        ...
        if (userInfo.role === "admin") {
            if (!pwAdminRequirement.test(newPw1) || !pwAdminRequirement.test(newPw2)) {
                return {
                    valid: false,
                    msg: "All password fields must be minimum 10 characters long",
                };
            }
        } else {
            if (!pwResearcherRequirement.test(newPw1) || !pwResearcherRequirement.test(newPw2)) {
                return {
                    valid: false,
                    msg: "All password fields must be minimum 8 characters long",
                };
            }
        }
        if (newPw1 !== newPw2) {
    ...
    function checkLoginFields() {
        ...
        if (email === "" || newPw1 === "" || newPw2 === "") {
            ...
            return {
                valid: false,
                msg: "This email is too long. Please choose a shorter one (below 64 characters).",
            };
        }
        // Check that the given mail contains an "@" and ".". Also ensures there are no whitespaces in the email (spaces, tabs, etc.)
            ...
            return {
                valid: false,
                msg: "This email is not valid. Please double check that it contains '@' and '.', and does not include whitespaces.",
            };
        }
        return {
            valid: true,
    ...
    const changeThePassword = () => {
        ...
        if (formIsValid) {
            ...
            userService
                ...
                .then((res) => {
                    if (res.status == 204) {
                        swal("Your password has been changed!", "You will now be logged out", "success").then(() => {
                        ...
                        swal("Unable to change password", res.data.error, "error");
                    }
                })
                .catch((err) => {
        ...
        <div class="d-flex flex-row justify-center">
            <div class="d-flex flex-column" style="width:50%;">
                ...
                <div class="d-flex flex-column justify-center align-left">
                    ...
                    >
                        Old Password
                        <div slot="append" on:click={() => (oldPasswordShow = !oldPasswordShow)}>
                    ...
                    <Button class="ma-2 mt-6" outlined on:click={changeThePassword}>Change Password</Button>
                </div>
            </div>
        </div>
    {:else}
        <div class="d-flex flex-column justify-center align-center" style="padding-top: 10vh;">
            <div class="d-flex flex-column">
                ...
                <h2 class="text-h4 mb-6 align-self-center">Log in below to see your surveys, and continue making new ones.</h2>
                <div class="d-flex flex-column justify-center align-left">
                    <TextField class="ma-2" rules={[validateEmail]} bind:value={email} on:keydown={loginOnEnterPress}>Email</TextField>
                    <TextField class="ma-2" bind:value={newPw1} type={newPw1Show ? "text" : "password"} on:keydown={loginOnEnterPress}>
                        Password
                        <div slot="append" on:click={() => (newPw1Show = !newPw1Show)}>
                            <Icon path={newPw1Show ? mdiEyeOff : mdiEye} />
                        </div>
