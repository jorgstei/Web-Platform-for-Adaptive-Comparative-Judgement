<script>
    ...
    function onFileSelected(e) {
        view = undefined;
        if (e?.target?.files[0]) {
            if(e.target.files[0]?.size > 16000000) //If file size is larget than 16MB
            {
                swal("File too large", "The selected file is too large. The maximum supported file size is 16MB.", "error")
                return;
            }
        ...
        if (view == undefined && typeof option._id == "string" && option._id.match(objectIdRegex)) {
            await surveyItemFileService.get(option._id).then((response) => {
                if (response.status < 300) {
                    ...
                    view = URL.createObjectURL(nodeBufferToFile(response.data.data, "application/pdf"));
