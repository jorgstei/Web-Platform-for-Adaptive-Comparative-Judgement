<script>
    ...
    const sendForgottenPasswordLink = () => {
        ...
        if (formIsValid) {
            userService
                .sendForgottenPasswordLink({ email: email })
                .then((res) => {
    ...
    const changeThePassword = () => {
        ...
        if (formIsValid) {
            ...
            userService
                .patchForgottenPassword({ email: email, newPassword: pw1, token: token })
                .then((res) => {
                    if (res.status == 204) {
        ...
        <div class="d-flex flex-column">
            ...
            {:else}
                <h2 class="text-h2 ma-2">
                    Please fill in the email address your account was created with, then create a new password and repeat it in the
                    following fields.
                </h2>
            {/if}
            ...
            <Button
                ...
                on:click={() => {
                    resetPassword ? changeThePassword() : sendForgottenPasswordLink();
                }}>{resetPassword ? "Change Password" : "Send reset password link"}</Button
            >
        </div>
    </div>
</main>
